...
""" % (test_name, result['loader_idx'],
    ...
    def run_workload(self, stress_cmd, nemesis=False, sub_type=None, cycles_count=-1):
        ...
        if nemesis:
            self.log.info("HJ: run_workload1")
            interval = self.params.get('nemesis_interval')
    ...
    def test_latency_write_with_nemesis(self):
        ...
        self.run_workload(stress_cmd=self.params.get('stress_cmd_w'), nemesis=True, sub_type='write', cycles_count=2)
        self.log.info("HJ: test_latency_write_with_nemesis6")

    def test_latency_mixed_with_nemesis(self):
        self.log.info("HJ: test_latency_mixed_with_nemesis1")
        self.run_fstrim_on_all_db_nodes()
        self.log.info("HJ: test_latency_mixed_with_nemesis2")
        self.preload_data()
        self.log.info("HJ: test_latency_mixed_with_nemesis3")
        self.wait_no_compactions_running(n=160)
        self.log.info("HJ: test_latency_mixed_with_nemesis4")
        self.run_fstrim_on_all_db_nodes()
        self.log.info("HJ: test_latency_mixed_with_nemesis5")
        self.run_workload(stress_cmd=self.params.get('stress_cmd_m'), nemesis=True, sub_type='mixed', cycles_count=2)
